...
export class Universe {
  ...
  tick() {
    ...
    for (let row = 0; row < this.height; row++) {
      for (let col = 0; col < this.width; col++) {
        ...
        if (cell.state) {
          if (live_neighbors < 2) {
            next_cell = new Cell(false);
          } else if (live_neighbors > 1 && live_neighbors < 4) {
            next_cell = new Cell(true);
  ...
  set_width(width) {
    self.width = width;
    self.cells = ([...Array(width * this.height)].map((i) => new Cell(false)));
  }
  
  set_height(height) {
    self.height = height;
    self.cells = ([...Array(this.width * height)].map((i) => new Cell(false)));
  }
